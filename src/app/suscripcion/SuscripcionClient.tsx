      ...
      <div className="max-w-6xl mx-auto space-y-10">
              ...
              <CardContent className="p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-amber-800 dark:text-amber-300 text-sm">⚠️ Pagos temporalmente deshabilitados</h3>
            ...
            return (
              <motion.div key={plan.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.15, duration: 0.5 }}>
                <Card className={`relative h-full flex flex-col transition-all duration-300 hover:shadow-xl ${
                  isPopular ? "border-primary shadow-lg scale-[1.02] md:scale-105" : "border-slate-200 dark:border-slate-800"
                  ...
                  <CardHeader className="pb-2 pt-6 text-center">
                    <div className="flex justify-center mb-3">
                      {isInstitutional ? <Building2 className="w-10 h-10 text-purple-500" /> : isPopular ? <Zap className="w-10 h-10 text-amber-500" /> : <Check className="w-10 h-10 text-slate-500" />}
                    </div>
                  ...
                  <CardContent className="flex-1 flex flex-col space-y-6">
                    <div className="text-center space-y-1">
                      <div className="flex items-baseline justify-center gap-1">
                        <span className="text-4xl font-extrabold tracking-tight">{formatPrice(plan.precio_centavos, plan.precio_mensual)}</span>
                        {hasRealPrice && <span className="text-muted-foreground">MXN/mes</span>}
                      </div>
                      {annualPrice > 0 && (
                        <p className="text-xs text-muted-foreground">o {formatPrice(plan.precio_centavos_anual, plan.precio_anual)} MXN/año</p>
                      )}
                      ...
                      {isCurrent ? (
                        ...
                        <>
                          <Button
                            ...
                            className={`w-full gap-2 ${isPopular ? "bg-primary hover:bg-primary/90" : ""} ${!canSubscribe ? "bg-slate-200 text-slate-500 cursor-not-allowed hover:bg-slate-200" : ""}`}
                          >
                          ...
                          {annualPrice > 0 && hasStripePrice && (
                            <Button onClick={() => handleSubscribe(plan, "annual")} disabled={isProcessing || !canSubscribe} variant="outline" className="w-full gap-2 text-xs">
        ...
        <div className="text-center space-y-2 pt-8 border-t border-slate-200 dark:border-slate-800">
          <p className="text-sm text-muted-foreground">
            Pagos procesados de forma segura por{" "}<span className="font-semibold text-slate-700 dark:text-slate-300">Stripe</span>.
          </p>
          <p className="text-xs text-muted-foreground">
            ¿Tienes dudas? Escríbenos a{" "}
            <a href="mailto:soporte@planeadocente.com" className="underline hover:text-primary transition-colors">soporte@planeadocente.com</a>
          </p>
        </div>
      </div>
    </div>
...
export default function SuscripcionClient() {
  return (
    <Suspense fallback={<SuscripcionSkeleton />}>
      <SuscripcionContent />
    </Suspense>
  );
}
